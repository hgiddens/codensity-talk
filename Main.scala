...
object Strict {
  ...
  def fullTree(i: Int): Tree[Int] = i match {
    ...
    case n =>
      ...
      for {
        t <- fullTree(j)
        node <- Node(Leaf(j - t), Leaf(t + 1)): Tree[Int]
      } yield node
  }
...
object Lazy {
  var leaves = 0
  var nodes = 0
  def tell = (nodes, leaves, nodes + leaves)
  def reset() = { leaves = 0; nodes = 0 }
  sealed trait Tree[A]
  case class Leaf[A](a: A) extends Tree[A] {
    leaves += 1
  }
    ...
    i match {
      ...
      case n =>
        ...
        fullTree[F](j).flatMap { t =>
          FT.node((j - t).point[F], (t + 1).point[F])
        }
        // for {
        //   t <- fullTree[F](j)
        //   node <- FT.node((j - 1).point[F], (j + 1).point[F])
        // } yield node
    }
...
object TreeBenchmark extends PerformanceTest {
  ...
  performance of "fullTree" in {
    measure method "strict" in {
      val strictTreeSizes = Gen.range("strict size")(2, 21, 1)
